...
# %%
"""
Autoencoder Anomaly Detection for Fracture Mapping
Trains an autoencoder on non-fracture pixels so it learns to reconstruct
"normal" deformation patterns.  Fracture pixels produce high reconstruction
...
# If no KMZ files are in the script directory, use the other data directory
...
def parse_ground_overlay (kml_root, zf):
    """Extract ground overlay images and their bounding boxes from a KML root."""
    overlays = []
...
def parse_placemarks (kml_root):
    """Extract LineString coordinates from Placemarks."""
    lines = []
...
print (f"\nBuilt {len (grids)} displacement grids: {list (grids.keys ())}")

# Select reference grid (EW)
...
def compute_spatial_features (z, pixel_spacing_x = 1.0, pixel_spacing_y = 1.0):
    z = z.astype (np.float32)
    features = {}

    grad_y, grad_x = np.gradient (z, pixel_spacing_y, pixel_spacing_x)
...
print (f"EW: {nodata_ew.sum ():,} NaN pixels ({100*nodata_ew.mean ():.1f}%) filled with 0")

# UP grid
z_orig_up = grid_up ["z"]
nodata_up = np.isnan (z_orig_up)
z_filled_up = np.where (nodata_up, np.float32 (0.0), z_orig_up).astype (np.float32)
print (f"UP: {nodata_up.sum ():,} NaN pixels ({100*nodata_up.mean ():.1f}%) filled with 0")
...
# Compute features one grid at a time, flatten immediately to save memory
def _compute_flat_features (z_f, H, W):
    """Compute 15 features (disp + 14 spatial) and return flat (H*W, 15) array."""
    feats = compute_spatial_features (z_f)
    stack = np.stack ([feats [f].astype (np.float32) for f in feature_order], axis = -1)
    X = np.concatenate ([z_f [:, :, np.newaxis], stack], axis = -1)
    return X.reshape (-1, X.shape [-1])

print ("Computing features (3 grids × 15 = 45 features, memory-efficient)...")
X_flat_ew = _compute_flat_features (z_filled_ew, H_ref := z_filled_ew.shape [0], W_ref := z_filled_ew.shape [1])
print (f"  EW features: {X_flat_ew.shape}")
X_flat_up = _compute_flat_features (z_filled_up, H_ref, W_ref)
print (f"  UP features: {X_flat_up.shape}")
X_flat_s1 = _compute_flat_features (z_filled_s1, H_ref, W_ref)
print (f"  S1 features: {X_flat_s1.shape}")
...
# Build 45-feature flat matrix from flat arrays
X_flat = np.hstack ([X_flat_ew, X_flat_up, X_flat_s1])
del X_flat_ew, X_flat_up, X_flat_s1  # free memory
...
print (f"Total features: {n_feat_total}, X_flat shape: {X_flat.shape}")
...
_artifact_file = os.path.join (DATA_DIR, "results", "artifact_mask.npz")
...
# Autoencoder training set: large sample of non-fracture pixels
AE_TRAIN_SIZE = 500_000
ae_train_size = min (AE_TRAIN_SIZE, len (nonfrac_idx))
ae_train_idx = rng.choice (nonfrac_idx, size = ae_train_size, replace = False)

# Test set: balanced sample of fracture + non-fracture for evaluation
# Exclude autoencoder training pixels from the test pool
ae_train_set = set (ae_train_idx)
nonfrac_remaining = np.array ([i for i in nonfrac_idx if i not in ae_train_set])

max_test_pos = min (len (frac_idx), 50000)
frac_test = rng.choice (frac_idx, size = max_test_pos, replace = False)
n_neg_test = min (len (nonfrac_remaining), 3 * len (frac_test))
nonfrac_test = rng.choice (nonfrac_remaining, size = n_neg_test, replace = False)

test_idx = np.concatenate ([frac_test, nonfrac_test])
rng.shuffle (test_idx)

X_ae_train_raw = X_flat [ae_train_idx]
X_test = X_flat [test_idx]
y_test = y_flat [test_idx]

print (f"\nAutoencoder training set: {len (X_ae_train_raw):,} non-fracture pixels")
print (f"Test set: {len (y_test):,} pixels "
      f"({ (y_test == 1).sum ():,} fracture, { (y_test == 0).sum ():,} non-fracture)")

scaler = StandardScaler ()
X_ae_train_sc = scaler.fit_transform (X_ae_train_raw)
...
# Anomaly threshold: 95th percentile of non-fracture reconstruction error
...
print (f"\n{"—"*70}")
print (f"  Autoencoder  |  F1 = {f1_ae:.4f}  |  AUC = {auc_ae:.4f}")
print (f"{"—"*70}")
...
with torch.no_grad ():
    for i in range (0, len (X_flat), CHUNK):
        chunk_sc = scaler.transform (X_flat [i:i+CHUNK])
...
axes [2].set_title (f"Autoencoder Predicted Fractures ({n_pred_ae:,} px)")

for ax in axes:
    ax.set_xlabel ("Longitude")
    ax.set_ylabel ("Latitude")

plt.suptitle (f"Autoencoder Anomaly Detection — {grid_name}", fontsize = 14, fontweight = "bold")
plt.tight_layout ()
# plt.show ()
...
ax.set_title (f"Autoencoder — Anomaly Map Overlaid on Displacement\n{grid_name}")
ax.set_xlabel ("Longitude")
ax.set_ylabel ("Latitude")
...
np.savez_compressed (os.path.join (_results_dir, "pred_autoencoder.npz"),
                    pred_map = pred_ae_map,
                    mse_map = mse_map,
                    threshold = ae_threshold,
                    model_name = "Autoencoder")
print (f"Saved Autoencoder -> {_results_dir}/pred_autoencoder.npz")
...
}, os.path.join (_models_dir, "autoencoder.pt"))
print (f"Saved AE weights -> {_models_dir}/autoencoder.pt")
